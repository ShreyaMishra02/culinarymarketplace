...
const AddressModal = () => {
  ...
  return (
    <>
      ...
      <Dialog open={showLoqate} onOpenChange={() => {}}>
        <DialogContent className="sm:max-w-md" onPointerDownOutside={e => e.preventDefault()} onEscapeKeyDown={e => e.preventDefault()}>
          <DialogHeader>
            <DialogTitle>Did you mean this address?</DialogTitle>
            <DialogDescription>We've validated your address. Please confirm.</DialogDescription>
          </DialogHeader>
          <div className="py-3">
            <div className="bg-muted rounded-lg p-3 text-sm space-y-0.5">
              <p className="font-medium">Suggested Address:</p>
              <p className="text-muted-foreground">{form.address1}</p>
              <p className="text-muted-foreground">{form.city}, {form.state} {form.zip}</p>
              <p className="text-muted-foreground">{form.country}</p>
            </div>
          </div>
          <div className="flex gap-3">
            <Button variant="outline" className="flex-1" onClick={handleAccept}>Keep My Entered Address</Button>
            <Button className="flex-1" onClick={handleAccept}>Use Suggested Address</Button>
          </div>
        </DialogContent>
      </Dialog>
    </>
