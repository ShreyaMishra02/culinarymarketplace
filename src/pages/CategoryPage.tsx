...
const flagFilters = [
  { key: "top" as const, label: "Top Products" },
  { key: "new" as const, label: "New Products" },
  { key: "discount" as const, label: "Discount Available" },
  { key: "bestselling" as const, label: "Best Selling" },
];
...
const CategoryPage = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter(p => {
      ...
      if (selectedFlags.length && !selectedFlags.some(f => p.badges.includes(f as any))) return false;
      ...
      <div>
        ...
        <div className="space-y-2">
          {category.subcategories.map(sub => (
            <label key={sub} className="flex items-center gap-2 cursor-pointer text-sm text-muted-foreground hover:text-foreground transition-colors">
  ...
  return (
    <div className="container-main py-6 sm:py-8">
      {/* Banner */}
      <div className="relative h-40 sm:h-48 rounded-lg overflow-hidden mb-6">
